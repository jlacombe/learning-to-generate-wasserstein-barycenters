...
def build_barycentric_line_figure(ins, targets, get_pbary, bweights, n_barys, 
    ...
    if targets is not None:
        rbarys = []
    if (display_inputs):
        merged_ins = ins[0,0].numpy().copy()#np.zeros(ins[0].shape, dtype=np.uint8)
    ...
    for i in range(n_barys):
        ...
        if targets is not None:
            ...
            rbary = gen_bary_from_targets(targets, bweights[i], bary_grid_size) #img_size
            rbary /= rbary.sum()
            rbary = rbary.numpy()
            rbarys.append(rbary)
    ...
    for i in range(n_barys):
        ...
        if targets is None:
            ...
            ax.imshow(pbary, vmin=vmin, vmax=vmax, cmap=cmap, alpha=alpha) # Greys, Reds, Blues
            plt.figure(figsize=(1,1),dpi=512);plt.imshow(pbary,vmin=vmin,vmax=vmax,cmap='Greys');plt.axis('off');plt.subplots_adjust(wspace=0, hspace=0, left=0, right=1, top=1, bottom=0);plt.savefig(os.path.join(folder_path,'pbary_{}.png').format(i))
        else:
            rbary = rbarys.pop(0)
            ...
            plt.figure(figsize=(1,1),dpi=512);plt.imshow(rbary,vmin=vmin,vmax=vmax,cmap='Greys');plt.axis('off');plt.subplots_adjust(wspace=0, hspace=0, left=0, right=1, top=1, bottom=0);plt.savefig(os.path.join(folder_path,'rbary_{}.png').format(i))
            plt.figure(figsize=(1,1),dpi=512);plt.imshow(pbary,vmin=vmin,vmax=vmax,cmap='Greys');plt.axis('off');plt.subplots_adjust(wspace=0, hspace=0, left=0, right=1, top=1, bottom=0);plt.savefig(os.path.join(folder_path,'pbary_{}.png').format(i))
